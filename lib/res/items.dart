List pizzaItems = [
  // {
  //   'image': 'asset/Margherit-removebg-preview.png',
  //   'itemName': 'Margherita',
  //   'rating': '4/5',
  //   'weight': 200,
  //   'detailShow': false,
  //   'isBookMark': false,
  //   'detail':
  //   "A hugely popular margherita, with a deliciously tangy single cheese topping",
  //   'prize': 200,
  // },
  //
  //
  // {
  //   'image': 'asset/Double_Cheese_Margherita-removebg-preview.png',
  //   'rating': '5/5',
  //   'detailShow': false,
  //   'isBookMark': false,
  //   'itemName': 'Double Cheese Margherita',
  //   'detail':
  //   "The ever-popular Margherita - loaded with extra cheese... oodies of it!",
  //   'weight': 300,
  //   'prize': 500,
  // },
  // {
  //   'image': 'asset/Farmhouse-removebg-preview.png',
  //   'rating': '4/5',
  //   'detailShow': false,
  //   'isBookMark': false,
  //   'itemName': 'Farmhouse',
  //   'detail':
  //   "A pizza that goes ballistic on veggies! Check out this mouth watering overload of crunchy, crisp capsicum, succulent mushrooms and fresh tomatoes",
  //   'weight': 250,
  //   'prize': 700,
  // },
  // {
  //   'image': 'asset/Peppy_Paneer-removebg-preview.png',
  //   'rating': '5/5',
  //   'detailShow': false,
  //   'isBookMark': false,
  //   'itemName': 'Peppy Paneer',
  //   'detail':
  //   "Chunky paneer with crisp capsicum and spicy red pepper - quite a mouthful!",
  //   'weight': 320,
  //   'prize': 450,
  // },
  // {
  //   'image': 'asset/Veg_Extravaganz.png',
  //   'rating': '4/5',
  //   'detailShow': false,
  //   'itemName': 'Mexican Green Wave',
  //   'detail':
  //   "A pizza loaded with crunchy onions, crisp capsicum, juicy tomatoes and jalapeno with a liberal sprinkling of exotic Mexican herbs.",
  //   'weight': 300,
  //   'prize': 500,
  // },
];
List saladsItem = [
  // {
  //   'image': 'asset/summer_somen_salad.png',
  //   'itemName': 'summer somen salad',
  //   'rating': '3/5',
  //   'isBookMark': false,
  //   'weight': 250,
  //   'detailShow': false,
  //   'detail':
  //   "For the dressing, whisk together the soy sauce, rice wine vinegar, sesame oil, sugar, and ginger. Set aside.",
  //   'prize': 225,
  // },
  // {
  //   'image': 'asset/Chilled_stir_fry_noodle.png',
  //   'itemName': 'Chilled stir fry noodle',
  //   'rating': '5/5',
  //   'weight': 250,
  //   'isBookMark': false,
  //   'detailShow': false,
  //   'detail':
  //   "For the dressing, whisk together the soy sauce, rice wine vinegar, sesame oil, sugar, and ginger. Set aside.",
  //   'prize': 325,
  // },
];
List dessertsItem = [
  // {
  //   'image': 'asset/choco-lava-cake.png',
  //   'itemName': 'choco lava cake',
  //   'rating': '4/5',
  //   'isBookMark': false,
  //   'weight': 100,
  //   'detailShow': false,
  //   'detail': "Filled with delecious molten chocolate inside.",
  //   'prize': 100,
  // },
  // {
  //   'image': 'asset/170046_BMC.png',
  //   'itemName': 'Butterscotch Mousse Cake',
  //   'rating': '5/5',
  //   'isBookMark': false,
  //   'weight': 125,
  //   'detailShow': false,
  //   'detail':
  //   "A Creamy & Chocolaty indulgence with layers of rich, fluffy Butterscotch Cream and delicious Dark Chocolate Cake, topped with crunchy Dark Chocolate morsels - for a perfect dessert treat!",
  //   'prize': 200,
  // },
];
List pastaItem = [
  // {
  //   'image': 'asset/penneallavodka.png',
  //   'isBookMark': false,
  //   'itemName': 'Italian Pasta',
  //   'rating': '4/5',
  //   'weight': 500,
  //   'detailShow': false,
  //   'detail':
  //   "Italian Pasta Italian vibes, but it’s cold out and you’re craving soup",
  //   'prize': 450,
  // },
];
List beveragesItem = [
  // {
  //   'image': 'asset/beverages.png',
  //   'isBookMark': false,
  //   'itemName': 'Drinks',
  //   'rating': '4/5',
  //   'weight': 100,
  //   'detailShow': false,
  //   'detail':
  //   "Milk , Tea , Coffee , Sparkling drinks , Juices , Energy drink , Mocktails , Milkshakes",
  //   'prize': 50,
  // },
];